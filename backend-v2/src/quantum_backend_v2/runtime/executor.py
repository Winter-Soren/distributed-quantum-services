...
# Note: These imports will need to be updated once the persistence and reservations modules are ported
# from quantum_backend_v2.persistence.runtime_store import (
#     FragmentExecutionEvent,
#     RuntimeEventStore,
# )
# from quantum_backend_v2.reservations.protocol import ReservationProtocol

# Temporary: Use typing.Protocol for compatibility
from typing import Protocol as TypingProtocol, Any

from quantum_backend_v2.planning.models import CircuitFragment, ExecutionPlan


class RuntimeEventStore(TypingProtocol):
    """Temporary protocol definition until persistence module is ported."""
    def append_fragment_event(self, event: Any) -> None: ...


class ReservationProtocol(TypingProtocol):
    """Temporary protocol definition until reservations module is ported."""
...
class FragmentExecutionEvent:
    """Temporary class definition until persistence module is ported."""
    def __init__(self, event_id: str, job_id: str, fragment_id: str, node_id: str,
                 attempt: int, status: str, error: str | None,
                 observed_fidelity: float | None, created_at: Any):
        ...
        self.status = status
        self.error = error
        self.observed_fidelity = observed_fidelity
        self.created_at = created_at
from quantum_backend_v2.runtime.gate_execution import GateExecutionAdapter
