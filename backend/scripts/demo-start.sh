#!/usr/bin/env bash
# Local startup script for backend
#
...
PEERSTORE_PATH="${PEERSTORE_PATH:-./quantum-backend/libp2p/peerstore.sqlite3}"

PEER_LOG_DIR="${QB2_PEER_LOG_DIR:-$(read_env_value QB2_PEER_LOG_DIR)}"
PEER_LOG_DIR="${PEER_LOG_DIR:-./quantum-backend/peer-logs}"
...
if [ -x ".venv/bin/quantum-backend" ]; then
  .venv/bin/quantum-backend --host 0.0.0.0 --port "$API_PORT"
else
  uv run quantum-backend --host 0.0.0.0 --port "$API_PORT"
fi
