<!-- BEGIN:frontend-v3-nextjs-agent-rules -->
...
Before starting this make sure you find and read the relevant docs in
`node_modules/next/dist/docs/`. Your training data is outdated — the docs are
the source of truth.
...
- `architecture.md` — repo-specific architecture, stack choices, auth, caching,
  and folder boundaries
- `DESIGN.md` — visual system, typography, motion, and interaction language
- `SKILL.md` — condensed checklist and rule index
- `AGENTS.md` — performance, security, and structural implementation rules

Conflict resolution:

- `architecture.md` chooses repo-specific stack and boundary decisions.
- `DESIGN.md` chooses look and feel.
- `AGENTS.md` chooses implementation quality, performance, and safety patterns
  inside that architecture.
- `SKILL.md` is a summary/index and must stay aligned with the other three
  documents.
...
5. [Re-render Optimization](#5-re-render-optimization) — **MEDIUM**
   - 5.1
     [Defer State Reads to Usage Point](#51-defer-state-reads-to-usage-point)
   - 5.2
     [Do not wrap a simple expression with a primitive result type in useMemo](#52-do-not-wrap-a-simple-expression-with-a-primitive-result-type-in-usememo)
   - 5.3
     [Extract Default Non-primitive Parameter Value from Memoized Component to Constant](#53-extract-default-non-primitive-parameter-value-from-memoized-component-to-constant)
   ...
   - 5.9
     [Use Transitions for Non-Urgent Updates](#59-use-transitions-for-non-urgent-updates)
...
8. [Advanced Patterns](#8-advanced-patterns) — **LOW**
   - 8.1
     [Enforce Module Structure and Pure UI Components](#81-enforce-module-structure-and-pure-ui-components)
   - 8.2 [Store Event Handlers in Refs](#82-store-event-handlers-in-refs)
   - 8.3
     [useLatest for Stable Callback Refs](#83-uselatest-for-stable-callback-refs)
...
### 8.1 Enforce Module Structure and Pure UI Components

**Impact: MEDIUM (Prevents logic leakage into UI components and enforces
predictable, refactor-safe module boundaries)**

React components frequently degrade over time when **data fetching, side
effects, and rendering logic are mixed together**.

This makes components harder to test, harder to reuse, and unsafe for automated
refactoring by agents.

To prevent this, each module or submodule must follow a **strict structural
contract**:

- `index.ts` — public exports only

- `*.tsx` — **UI / presentational only**

- `*.actions.ts` — **logic only** (hooks, side effects, data fetching)

UI components must be **pure**: props in → JSX out.

---

**Incorrect: UI handles logic and side effects**
...
**Correct: Pure UI with isolated logic**
...
import type { Block } from './home.actions';
...
export { useHomeBlocks } from './home.actions';

// page or route-level usage
import { Home, useHomeBlocks } from '@/modules/home';
...
<!-- END:frontend-v3-nextjs-agent-rules -->