...
description:
  ...
  coral, dark green, peach, and dark navy that punctuate long-scroll explainer

  pages. Primary actions use a near-black pill CTA; secondary actions sit in a

  white outlined button. Type runs Haas Grotesk in modest weights — never bold
...
components:
  ...
  button-legal:
    ...
    backgroundColor: '{[colors.link](http://colors.link)}'

    textColor: '{colors.on-primary}'

    typography: '{[typography.legal](http://typography.legal)}'
  ...
  text-link:
    ...
    textColor: '{[colors.link](http://colors.link)}'
